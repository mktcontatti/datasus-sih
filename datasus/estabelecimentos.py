...
def _buscar_direto(cnes: str) -> tuple[str | None, str | None] | None:
    """Busca por CNES exato e rejeita respostas de outro estabelecimento."""
    resposta = requests.get(API_BASE, params={"codigo_cnes": int(cnes), "limit": 10}, timeout=TIMEOUT)
    resposta.raise_for_status()
    for item in _itens(resposta.json()):
